...
class RouteController:
    def __init__(self, nodes: dict, planned_routes: list):
        """
        初始化

        :param nodes: 地图节点信息
        :param planned_routes: 每车规划路径
        """
        # 全局常量
        self.nodes = nodes  # map路径信息
        self.num_of_agv = 8  # agv数量

        # 路径规划相关
        self.residual_routes = planned_routes  # 车辆规划路径（包含转弯耗时）, dict

        # 全局变量
        self.reservation = None  # PR哈希, -1 -> 未预定, >=0 -> 被对应编号预定
        self.hash_route = None  # list
        self.update_shared_flag = [True for _ in range(self.num_of_agv)]

        # 计算相关
        # self.total_shared_routes = self._init_total_shared_routes()  # dict
        self.shared_routes = None  # dict
        self.shared_agvs = None  # dict
    ...
    def _update_reservation(self, loc_list: list) -> None:
        """利用当前位置更新预定，仅保留当前位置"""
        self.reservation = [-1 if i not in loc_list else loc_list.index(i) for i in range(len(self.nodes))]
    ...
    def update_routes(self, routes: dict):
        ...
        for key, route in routes:
            ...
            logging.info(f'AGV{key} route updated.')
        # 更新 hash_route 和 residual_route
        self._init_shared_routes()

    def get_instruction(self, status_list: list, loc_list: list, step_list: list) -> list:
        """
        返回每辆车的控制策略（以列表形式）

        :param status_list: 每辆车的当前状态，0-1变量表示车辆当前是否可以前进，len=8
        :param loc_list: 当前所有车辆位置（节点编号），len=8
        :param step_list: 表示在当前节点是否前进，用以更新 residual routes，len=8
        :return: 车辆控制策略列表
        """
            ...
            # 有剩余路径
                ...
                if step_list[agv]:
                    if step_grid_list[agv] in self.shared_routes[agv]:
                        ...
                        self.shared_routes.pop(del_idx)
                        self.shared_agvs.pop(del_idx)
                # check 其他车辆前进
                for idx in range(len(self.shared_routes[agv])):
                    # update due to moving forward
                    if self._is_list_contained(self.shared_agvs[agv], self.hash_route[self.shared_routes[agv][idx]]):
                        assert len(self.hash_route[self.shared_routes[agv][idx]]) == len(self.shared_agvs[agv]) + 1
                    else:
                        # delete
